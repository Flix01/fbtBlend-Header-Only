...
enum BlenderObjectType {
    BL_OBTYPE_EMPTY         =  0,
    BL_OBTYPE_MESH          =  1,
    BL_OBTYPE_CURVES_LEGACY =  2,	// used to be BL_OBTYPE_CURVE before Blender 3.1 (deprecated: will be replaced by BL_OBTYPE_CURVES)
    BL_OBTYPE_SURF          =  3,
    BL_OBTYPE_FONT          =  4,
    BL_OBTYPE_MBALL         =  5,
    ...
    BL_OBTYPE_LAMP          = 10,
    BL_OBTYPE_CAMERA        = 11,
  	BL_OBTYPE_SPEAKER 		= 12,
  	BL_OBTYPE_LIGHTPROBE 	= 13,
        
    BL_OBTYPE_WAVE          = 21,    // not sure if this is still in use...    
    BL_OBTYPE_LATTICE       = 22,
        
    BL_OBTYPE_ARMATURE      = 25,
   	BL_OBTYPE_GPENCIL 		= 26,	// in 3D view (but no 2D annotation).
  	BL_OBTYPE_CURVES 		= 27,	// from Blender>=3.1 it should contain: hair. It will replace BL_OBTYPE_CURVES_LEGACY too. 
  	BL_OBTYPE_POINTCLOUD 	= 28,
  	BL_OBTYPE_VOLUME 		= 29,   
...
int main(int argc, const char* argv[]) {
    ...
    /* From the declaration of struct fbtBlend (in fbtBlend.h):
	fbtList m_scene;
	fbtList m_library;
	fbtList m_object;
	fbtList m_mesh;
